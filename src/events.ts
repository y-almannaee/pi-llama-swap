...
import { mergeUpstreamMeta } from "./tools/cache";
 ...
 * Also triggers upstream metadata fetch for the selected model.
 */
export const onModelSelect = async (
  ...
  // Fire-and-forget: fetch upstream metadata only for the active model.
  // Safe — checks /running first, only uses /upstream/:id proxy when
  // the model is already loaded (avoids triggering a model swap).
  void mergeUpstreamMeta().catch(() => {});
};
